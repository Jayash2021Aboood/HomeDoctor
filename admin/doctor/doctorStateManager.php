  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') 
  {
    ...
    if(isset($_POST['changeStateToAccept']))
    {
      ...
      if(count($errors) == 0)
      {
        ...
        $update = $update = query("UPDATE doctor SET state = 'accept' WHERE id = $id");
        if($update ==  true)
        {

          redirectToRefererSuccess("Doctor Accepted successfuly!");
        }
        else
        {
    ...
    if( empty($state)){
      ...
      if(count($errors) == 0)
      {
        ...
        $update = query("UPDATE doctor SET state = 'reject' WHERE id = $id");
        if($update ==  true)
        {

          redirectToRefererSuccess("Doctor Rejected successfuly!");
        }
        else
        {
          redirectToReferer("Error when Update Data");
